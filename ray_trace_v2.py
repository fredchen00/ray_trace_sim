...
def generate_rays(init_pos,fibre=mmf_fibre,mesh_density = 50, num_rays = 1000**2): 
	...
	if n_iter1>0:
		refined_density = (num_rays/n_iter1)**0.5*mesh_density
...
def gen_sim_imgs(x_pos,y_pos,z_pos,num_rays_var):
	...
	xyz,rays = generate_rays([x_pos,y_pos,z_pos],num_rays=num_rays_var);
	if len(xyz)==0:
		print 'No ray generated. Skip.'
		return np.zeros([200,200])	   
	print 'number of rays: '+str(len(xyz));
	f_pos=propagate_multithread(rays,xyz[:,:2]);
	heatmap, xedges, yedges = np.histogram2d(f_pos[:,0], f_pos[:,1], bins=200)
	norm_heatmap=np.floor((heatmap/np.max(np.max(heatmap)))*2**16);
...
if __name__=='__main__':
	num_threads=sim_config.num_threads
	save_dir=sim_config.save_dir
	num_rays=sim_config.num_rays
	step_size=sim_config.step_size
	grid_size=sim_config.grid_size
	offset_index=sim_config.offset_index
	z_pos=sim_config.z_pos
	start = time.time()
	origin_pos=[-mmf_fibre.a,-mmf_fibre.b]
	
	
	for y_index in range(offset_index[0],grid_size[0]):
		y_pos=y_index*step_size+origin_pos[1];
		for x_index in range(grid_size[1]):
			if ~((y_index==offset_index[0]) & (x_index<offset_index[0])):
				x_pos=x_index*step_size+origin_pos[0]
				print 'Pos:'+str(x_pos)+','+str(y_pos);
				img_array=gen_sim_imgs(x_pos,y_pos,z_pos,num_rays)
				file_name='sim_img'+str(y_index)+'_'+str(x_index)+'.tif'
